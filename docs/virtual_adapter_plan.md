...
**Sprint Playbook (a.k.a. how we stop the ghosts):**
1. **Broaden the command dialect.** Fold `ObjectBlit` and `UiBlit` payloads into `render_commands`, mirroring the tile schema but carrying critter-facing metadata (frame, rotation, scripted tint). Keep serialization/replay parity so frame dumps stay debuggable.
2. **Wire every legacy blitter into the bus.** `_obj_render_object`, UI widgets, and stray particle scribblers must emit commands whenever the orchestrator flag is hot, falling back to legacy overlay writes only when the Phantom Display naps.
3. **Teach the orchestrator new choreography.** Extend `render_display_orchestrator.cc` to consume the fresh ops, resolve HD vs. fallback assets, rebuild flat/per-pixel lighting, and paint both logical + physical overlays without tile-specific shortcuts. Update coverage counters so we can prove HD really won the frame.
4. **Instrument, guard, and test.** Add `SCALER orchestrator objects=… ui=…` counters, expand the replay harness to validate the new payloads, and drop assertions/logs whenever an object bypasses the dispatcher. Finish with hover/combat/UI stress runs so QA isn’t chasing phantom flicker reports.
...
- [x] **S6.4** Build the real-display orchestrator.
	...
	- Presenter treaty locked in: once the Neon Dispatcher owns the frame, `windowPresentVirtualScreen` wipes the SDL texture (`SCALER presenter_clear reason=orchestrator ...`) before copying overlays, so the low-res `_screen_buffer` never leaks to the glass. The virtual cache keeps feeding the classic engine, but the real display only shows pixels sourced from the orchestrator’s HD/fallback caches.
	- Legacy escape hatch remains for boot/loading screens: if the tile window naps (menus, movies, splash loads), the orchestrator drops presenter ownership so `windowPresentVirtualScreen` falls back to the classic `_screen_buffer` copy. That keeps UX visible until we migrate UI/intro paths into the command dialect.
	- First UI scouts have landed: every `artRender` call now resolves its owning window, registers the FRM with the asset registry, and emits a `RenderCommandOp::UiBlit` tagged with that window id. The orchestrator’s window map reroutes those ops into the correct true-color buffers (tile, menu, loading), so the presenter finally paints even when no iso scene exists—HD when available, auto-converted RGBA when not.
	- Mission follow-up: the Neon Dispatcher now actually replays those `UiBlit` packets. `render_display_orchestrator` clears each targeted window the moment it seizes control, composites the HD/fallback pixels into their physical overlays, and keeps the SDL presenter scrubbed unless we intentionally surrender it for movies or other legacy detours. Menu, loading, and splash scenes now live entirely on the Phantom Display instead of leaking `_screen_buffer` to glass.
...
- [x] **S6.7** Stretch past 1:1 shackles.
	- HdTrueColorFrameView now carries texel-origins + per-axis ratios, the loader happily slurps oversized PNGs, and the palette downsampler samples bilinearly so modders can sneak in oddball margins without confusing the Neon Archivist.
	- The Neon Dispatcher’s orchestrator blitters map logical spans directly into those UVs, bilinearly sample every physical pixel, and only clamp detail when the viewport physically can’t keep up.
	- `SCALER orchestrator ... hd_scale_max=%.2f` now tattles on every frame, making it obvious whether the window is the bottleneck or the asset packed low-res sludge.
