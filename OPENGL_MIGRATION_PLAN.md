...
1.  **Build System (`CMakeLists.txt`)**: [COMPLETED]
...
2.  **Create `OpenGLContext`**: [COMPLETED]
...
3.  **Shader Management**: [IN PROGRESS]
...
1.  **Update `RenderPipeline`**: [COMPLETED]
...
2.  **Port Shader Passes**: [COMPLETED]
...
1.  **Asset Loading**: [COMPLETED]
    *   Ensure `.glsl` files are copied to the build output directory.
2.  **Shader Management**: [COMPLETED]
    *   Implement a GLSL shader loader that reads `.glsl` files directly from the `data/shaders` directory.
    *   Compile and link Compute Shaders at runtime.
3.  **Hot Reloading (Optional)**: [PENDING]
